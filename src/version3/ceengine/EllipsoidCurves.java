 ...
 * Calculates q^6 times the scattering intensity as a function of q for ellipsoids of various pairs of radii.
 *
 ...
 *             ...rN||r1......rN||r1......rN||   ||r1......rN||r1......rN||r1......rN]
 *
 * @author Nathan
 * @version 1.0
 */
public class EllipsoidCurves extends PredictedIntensityCurves {

    private int noThreads = 4;
    private int qPerThread;
    private float scale;

    public EllipsoidCurves(float scale){
    ...
    public void setThings(ArrayList<ParameterSet> paramValues, double[] qValues) {             //USES double[] FOR THE Q VALUES AS THIS IS HOW IT COMES FROM XYSERIES
        ...
        for (int i = 0; i < paramLength; i++) {
            for(int j = 0; j < paramValues.get(0).getParamNumbers(); j++){
    ...
    private void fillValues(float[] rValues, double[] qValues) {
        ...
        for (int i = 0; i < noThreads-1; i++){
            Callable<float[]> callable=new EllipsoidQCallable(i, rValues, i*qPerThread, (i+1)*qPerThread, qValues, paramLength);
            Future<float[]> future= CrossEntropyEngine.pool.submit(callable);
            set.add(future);
        }
        Callable<float[]> callable=new EllipsoidQCallable(noThreads-1, rValues, (noThreads-1)*qPerThread, (qValues.length), qValues, paramLength);
        Future<float[]> future=CrossEntropyEngine.pool.submit(callable);
        set.add(future);
