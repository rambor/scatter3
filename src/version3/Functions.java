...
//       int bins = (int)(Math.round(qmax*dmax/Math.PI));
    ...
    /**
     * Performs real space indirect Fourier Transform using P. Moore Method
     * @param data XYSeries containing q and I(q) | note anti-log
     * @param error XYSeries containing q and uncertainties for I(q)
     * @param dmax Maximum dimension
     * @return ArrayList[0] contains SAXS invariants <br>
     *         ArrayList[0][0] Izero<br>
     *         ArrayList[0][1] error for Izero<br>
     *         ArrayList[0][2] Rg<br>
     *         ArrayList[0][3] error for rg<br>
     *         ArrayList[0][4] <r>, Mean distance<br>
     *         ArrayList[0][5] error for <r><br>
     *         ArrayList[1] contains Moore coefficients<br>
     *         ArrayList[2] Error for the coefficients<br>
     */
    public static ArrayList<ArrayList<Double>> moore_pr(XYSeries data, XYSeries error, double dmax){
   /*
    ...
    * n: columns
    *
    * Zero element is a(0) for background/offset correction
    */
        rows = data.getItemCount();

        cols = nLimit+1; //total number of Moore coefficients + a(0)
        LinearSolver<DenseMatrix64F> solver = LinearSolverFactory.leastSquares(rows, cols);
        // Begin Solve Method from Moore paper
        SimpleMatrix cmn = new SimpleMatrix(cols,cols);
        SimpleMatrix ym = new SimpleMatrix(cols,1);
        double bm;
        double bn;
        double s_obs_at;
        double y_obs_at;
...
//            if(Scatter.refineStopped){
//                break;
//            }
            pi_sq_m = n_pi_squared[m];
...
//                        break;
//                    }
                    x_obs_at = data.getX(i).doubleValue();
                    y_obs_at = data.getY(i).doubleValue();
                    s_obs_at = error.getY(i).doubleValue();
                    sin_x_dmax = Math.sin(x_obs_at*dmax)*pi_dmax;  //move to a LUT or memoization
                    inv_sx = 1;
                    xdmax = Math.pow(x_obs_at*dmax,2);
                    if (m==0){
                        bm = 1;
                    } else {
                        bm = m*Math.pow(-1.0, m+1)*sin_x_dmax/(pi_sq_m-xdmax);
                    }

                    if (n==0){
                        bn = 1;
                    } else {
                        bn = n*Math.pow(-1.0, n+1)*sin_x_dmax/(pi_sq_n-xdmax);
                    }

                    sumYm =  sumYm  + inv_sx*x_obs_at*y_obs_at*bm; //error weighting
                    sumCmn = sumCmn + inv_sx*bm*bn;                //error weighting
                }
                //add to matrix
                ym.set(m,0, sumYm);
                cmn.set(m, n, sumCmn);
        ...
        SimpleMatrix cmnInvert = cmn.invert();

    /*
     * Use EJML solver to find Moore Coefficients instead of direct matrix inversion
     */
        DenseMatrix64F testMatrix = new DenseMatrix64F(cmn.numCols(),1);
        //solver.setA(cmn.getMatrix());
        if( !solver.setA(cmn.getMatrix()) ) {
            System.out.println("Exception found with Matrix Inversion");
            resultsM.add(new ArrayList<Double>(Collections.nCopies(6, 0.0)));
            resultsM.add(new ArrayList<Double>(Collections.nCopies(cols, 0.0)));
            resultsM.add(new ArrayList<Double>(Collections.nCopies(cols, 0.0)));
            return resultsM;
        }

        solver.solve(ym.getMatrix(),testMatrix);
...
//            }
            if (i != 0) {
                i_zero = i_zero + testMatrix.get(i,0)/(i)*Math.pow(-1,(i+1));
                partial_rg = partial_rg + testMatrix.get(i,0)/Math.pow(i,3)*(Math.pow(Math.PI*i, 2) - 6)*Math.pow(-1, (i+1));
                rsum = rsum + testMatrix.get(i,0)/Math.pow(i,3)*((Math.pow(Math.PI*i, 2) - 2)*Math.pow(-1, (i+1)) - 2 );
            }
        }
...
//                break;
//            }
            a_m.add(testMatrix.get(i,0));  //Moore method using Normal Eqns from Chi2
        }
        double dmax2 = dmax*dmax;
        double dmax3 = dmax2*dmax;
        double dmax4 = dmax2*dmax2;
        ...
        i_zero = i_zero*dmax2/Math.PI;
        rg = Math.sqrt(dmax4*inv_pi_cube/i_zero*partial_rg)*0.7071067811865475; // 1/Math.sqrt(2);
        average_r = dmax3*inv_pi_cube/i_zero*rsum;

    /*
     * Calculate error for Izero, average_r
     */

        error_am.add(cmnInvert.get(0, 0));
        double inv_i3_pi_sq;
        double rg_2 = Math.pow(rg,2);
        double inv_dmax = average_r*1/dmax;
        double inv_dmax2 = rg_2*2*1/dmax2;
        double inv_j3_pi_sq;
        double pi_i_2;
        double pi_j_2;
        for (int i = 1; i < cmnInvert.numRows(); i++) {
//            if(Scatter.refineStopped){
//                break;
//            }
            error_am.add(cmnInvert.get(i,i));
            //error_am.add(inv_normal.get(i,i));
            inv_i3_pi_sq = 1/(Math.pow(i,3)*pi_sq);
            pi_i_2 = Math.pow(Math.PI*i,2);
            e_n = inv_i3_pi_sq*((pi_i_2-2)*Math.pow(-1,(i+1))-2) - inv_dmax/i*Math.pow(-1,(i+1));
            f_n = inv_i3_pi_sq* (pi_i_2-6)*Math.pow(-1,(i+1)) - inv_dmax2/i*Math.pow(-1,i+1);

            for (int j = 1; j < cmnInvert.numCols(); j++) {
//                if(Scatter.refineStopped){
//                    break;
//                }
                inv_j3_pi_sq = 1/(Math.pow(j,3)*pi_sq);
                pi_j_2 = Math.pow(Math.PI*j,2);
                e_m = (inv_j3_pi_sq*((pi_j_2-2)*Math.pow(-1,(j+1))-2) - inv_dmax/j*Math.pow(-1,(j+1)));
                f_m =  inv_j3_pi_sq* (pi_j_2-6)*Math.pow(-1,(j+1)) - inv_dmax2/j*Math.pow(-1,j+1);
                error_r = error_r + e_n*e_m*cmnInvert.get(i,j);
                error_rg = error_rg + f_n*f_m*cmnInvert.get(i, j);
                i_zero_error = i_zero_error + 1/(j*i)*Math.pow(-1,(j+i))*cmnInvert.get(i, j);
            }
        }

        i_zero_error = dmax2*Math.sqrt(i_zero_error)*inv_pi*inv_pi*0.25;
        error_r = dmax3/i_zero*Math.sqrt(error_r)*inv_pi*0.25;
        // Rg/M*1/sqrt(2)*sigma_M
        error_rg = rg/(dmax4*inv_pi_cube/i_zero*partial_rg)*0.7071067811865475*dmax4/i_zero*Math.sqrt(error_rg)*inv_pi*0.25*inv_pi*0.5;
        // error propagation, error_rg is actually the second moment -> sqrt(error_rg)/sqrt(2)

        constants.add(i_zero);
        constants.add(i_zero_error);
        constants.add(rg);
        constants.add(error_rg);
        constants.add(average_r);
        constants.add(error_r);
        resultsM.add(constants);
        resultsM.add(a_m);
        resultsM.add(error_am);
        return resultsM;
    }
    ...
    public static double moore_Iq_L(ArrayList<Double> a_m, double dmax, double q, int limit, boolean background){
        ...
        for(int i=0; i < limit; i++){
            ...
            if (i==0 && background){
                resultM = a_m.get(0)*invq;
            }  else {
                resultM = resultM + a_m.get(i)*Math.PI*i*dmax*Math.pow(-1,i+1)*Math.sin(dmax*q)/(Math.pow(Math.PI*i, 2) - Math.pow(dmax*q, 2))*invq;
            }
        }
    ...
    /**
     * Calculates P(r) at a single r value
     * ArrayList<Double> of coefficients,
     * @param a_m Moore coefficients
     * @param dmax Maximum dimension
     * @param r r value for which you want to calculate P(r)
     ...
     */
        ...
        return 1.0/Math.PI*0.5*r*resultM;
    }
    ...
    public static double medianMooreResiduals(XYSeries data, double[] a_m, double dmax){
        ...
        for (int i = 0; i < dataLimit; i++){
            resultM = 0.0;
            q = data.getX(i).doubleValue();
            dmax_q2 = Math.pow(dmax*q, 2);
            sin_dmaxq = Math.sin(dmax*q);
            for(int j=0; j < limit; j++){
                if (j== 0){
                    resultM = a_m[0];    //constant background term
                }  else {
                    resultM = resultM + a_m[j]*Math.PI*j*dmax*Math.pow(-1,j+1)*sin_dmaxq/(Math.pow(Math.PI*j, 2) - dmax_q2);
                }
            }
            residuals.add( Math.pow(data.getY(i).doubleValue() - resultM/data.getX(i).doubleValue(), 2) );  //squaring the difference
        }
        ...
        for (int i = 0; i < dataLimit; i++){
            ...
            for(int j=0; j < limit; j++){
                if (j== 0){
                    ...
                    resultM = resultM + a_m.get(j)*Math.PI*j*dmax*Math.pow(-1,j+1)*Math.sin(dmax*q)/(Math.pow(Math.PI*j, 2) - Math.pow(dmax*q, 2));
                }
            }

            residuals.add(Math.pow(1/error.getY(i).doubleValue(),2)*Math.pow(Math.pow(10,data.getY(i).doubleValue()) - resultM/data.getX(i).doubleValue(),2));
        }
    ...
    public static double[] leastSquares(double[] x_data, double[] y_data){
        ...
        s_e_2 = 1/(n*(n-2))*(n*sumYsq - Math.pow(sumY,2) - Math.pow(m,2)*(n*sumXsq - sumX2));
        s_m_2 = n*s_e_2/(n*sumXsq- sumX2);
        s_b_2 = s_m_2/n*sumXsq;
    ...
    public static double[] autoRg(XYSeries data, XYSeries errors, int startAt) {
        ...
        XYDataItem lastItem;
        while( tempDataItem.getXValue() < 0.18 && first < (last-10)){
            ...
            while (last > first+7) {
                ...
                tempRg = Math.sqrt(-3.0*slope);

                // find the largest gap with qRg < 1.5
                if ((last-first) > deltaGap && errors.getDataItem(last).getXValue()*rg < 1.5){
                    deltaGap = last - first;
                    rg = tempRg;
                    endAt = last;
                }
        ...
        double[] x_data = new double[endAt];
        double[] y_data = new double[endAt];
        ...
        for (int i=startAt; i < endAt; i++) {
            ...
            x_data[i] = xvalue;  // already q^2
            //ln(y)
            y_data[i] = yvalue;
            //ln(y)*error*y
            //w_data[arrayIndex] = y_data[arrayIndex] * errors.getY(i).doubleValue() * yvalue;
        }
        ...
        // Estimate Rg to set q*Rg limit for data range
        /*
        while((endIndex - forward) > 7){

            x_range = Functions.subSelectarray(x_data, forward, endIndex);
            y_range = Functions.subSelectarray(y_data, forward, endIndex);

            double[] param = Functions.leastSquares(x_range, y_range);
            c1 = param[0];
            c0 = param[1];

            for (int v=0; v < arrayIndex; v++) {
                residuals3[v] = (y_data[v]-(c1*x_data[v]+c0))*(y_data[v]-(c1*x_data[v]+c0));
            }
        ...
        */
        ...
        if (sizeOfArray > 5 ) { // if too few points, just fit the first 5

            int truncate_head = 7; // ignores the leading points near beamstop
            x_range = new double[sizeOfArray - truncate_head];
            y_range = new double[sizeOfArray - truncate_head];
            w_range = new double[sizeOfArray - truncate_head];

            getDataLoop:
            for (int i = truncate_head; i < sizeOfArray; i++) {
                XYDataItem dat = data.getDataItem(i);
            ...
            double[] keptResiduals = new double[arrayIndex];

            while (trial < 5000) {
                //subsample = (int)(Math.random()*1000000)%(arrayIndex-4) + 3;
                subsample = 5;
                int[] random_index = Functions.rand_n(subsample, arrayIndex - 1);
                ...
                if (median_test < minResidual) {
                    minResidual = median_test;
                    System.arraycopy(residuals3, 0, keptResiduals, 0, arrayIndex);
                }
                trial++;
            }
            ...
            double s_o = 1.4826 * (1.0 + 5.0 / (arrayIndex - 2)) * Math.sqrt(minResidual);
            double inv_s_o = 1.0/s_o;

            double temp_resi, root_resi;
            double sum_wr2 = 0.0, sum_w = 0.0;

            ArrayList<Double> median_abs_resi = new ArrayList<Double>();
            for (int i = 0; i < arrayIndex; i++) {

                temp_resi = keptResiduals[i];
                root_resi = Math.sqrt(temp_resi);
                median_abs_resi.add(root_resi);

                if ((root_resi * inv_s_o) < 2.5) {
                    sum_wr2 += temp_resi;
                    sum_w += 1;
                }
            }
            ...
            double sigma = Math.sqrt(sum_wr2 / (sum_w - 2.0));
            double inv_sigma = 1.0 / sigma;
            count = 0;
            ArrayList<Integer> keepers = new ArrayList<Integer>();
            ArrayList<Double> forMAD = new ArrayList<Double>();

            for (int i = 0; i < arrayIndex; i++) {
                temp_resi = keptResiduals[i];
                if (Math.sqrt(temp_resi) * inv_sigma < 2.5) {
                    keepers.add(i);
                    count++;
                }
                forMAD.add(y_range[i]);
            }

            //System.out.println("Rejected: " + (arrayIndex - count) + " points from Guinier region");
            //System.out.println("Percent rejected " + (arrayIndex - count) / (double) arrayIndex);
            ...
            for (int i = 0; i < count; i++) {
                keep = keepers.get(i);
                final_x[i] = x_range[keep];
                final_y[i] = y_range[keep];
                final_w[i] = w_range[keep];
            }
    ...
    /**
     * AutoRg algorithm for calculating I(zero) and Rg
     * @param data - XYSeries must be nonNegative
     * @param errors - associated errors as XYSeries
     * @return array of doubles
     */
    public static double[] calculateIzeroRg(XYSeries data, XYSeries errors) {

        if(data.getItemCount()==0 || errors.getItemCount()==0){
            System.out.println("XYSeries data or XYSeries error is empty from calculateIzeroRg()");
            return null;
        }
        ...
        // number of potential data points less than qLimit (for initializing array);
        countingLoop:
        for(int i=0; i< itemCount; i++){
            XYDataItem dat = data.getDataItem(i);
            xvalue = dat.getXValue();
            yvalue = dat.getYValue();
            if ((xvalue < qLimit) && (yvalue > 0)) {
                sizeOfArray++;
            } else if (xvalue >= qLimit) {
                break countingLoop;
            }
        }

        double[] x_data = new double[sizeOfArray];
        double[] y_data = new double[sizeOfArray];
        ...
        for (int i=0; i < itemCount; i++) {
            ...
            yvalue = dat.getYValue();
            //x^2
            if ((yvalue > 0) && (xvalue < qLimit)) {
                //x_data[i] = Math.pow(xvalue, 2);
                x_data[arrayIndex] = xvalue*xvalue;
                //ln(y)
                y_data[arrayIndex] = Math.log(yvalue);
                //ln(y)*error*y
                //w_data[arrayIndex] = y_data[arrayIndex] * errors.getY(i).doubleValue() * yvalue;
                //original data
                arrayIndex++;
        ...
        double[] residuals3 = new double[arrayIndex];
        ...
        for(int i=0; i < itemCount; i++){
            ...
            xvalue = dat.getXValue();
            yvalue = dat.getYValue();
            if ((xvalue*rg < 1.3) && (yvalue > 0)) {
                sizeOfArray++;
            } else if (xvalue*rg >= 1.3) {
                break countingLoop;
            }
        }
        ...
        if (sizeOfArray > 5 ) {
            ...
            x_range = new double[sizeOfArray - truncate_head];
            y_range = new double[sizeOfArray - truncate_head];
            double[] w_range = new double[sizeOfArray - truncate_head];
            ...
            for (int i = truncate_head; i < sizeOfArray; i++) {
                ...
                if ((xvalue * rg < 1.3) && (yvalue > 0)) {
                    //x_data[i] = Math.pow(xvalue, 2);
                    x_range[arrayIndex] = Math.pow(xvalue, 2);
                    //ln(y)
                    y_range[arrayIndex] = Math.log(yvalue);
                    //ln(y)*error*y
                    w_range[arrayIndex] = Math.log(yvalue) * errors.getY(i).doubleValue() * yvalue;
                    //original data
                    arrayIndex++;
                } else if (xvalue * rg >= 1.3) {
                    break getDataLoop;
    ...
    public static double[] complexation(double[] yVector, ArrayList<double[]> designMatrix){
        ...
        LinearSolver<DenseMatrix64F> solver = LinearSolverFactory.leastSquares(rows, constants);
        // Begin Solve Method from Moore paper
    ...
    public static ArrayList<Double> svdReduce(Scatter.PrModel prModel, int total, int dmax, int selectedSize, int coefSize){
        ...
        LinkedHashMap sorted = Functions.sortHashMapByValuesD(izeros);
...
//        V.print();
        ...
        ArrayList<Double> invariants = Functions.saxs_invariants(mooreCoefs, dmax);
...
*/
    ...
    public static XYSeriesCollection svdNoiseReduce(Collection dataCollection) {
        ...
        int totalSets = dataCollection.getDatasets().size();
        int firstIndex=0;
        ...
        if (flag && flagX){
            ...
            for(int i=0; i<totalSets; i++){
                ...
                if (dataCollection.getDataset(i).getInUse()){
                    ...
                    for(int w=0; w<totalInTemp; w++){
                        ...
                        tempqmin = tempData.getX(w).doubleValue();
                        if (tempqmin >= qmin && tempqmin <= qmax){
                            matrixA.set(row, col, tempData.getY(w).doubleValue());
                            tempValue = errorData.getY(w).doubleValue();
                            variances.set(row,0, variances.get(row,0) + 1.0/(tempValue*tempValue));
                            row++;
                        }
                    }
                    col++;
            ...
            SingularValueDecomposition<DenseMatrix64F> svd = DecompositionFactory.svd(rows, trueCount, true, true, false);
