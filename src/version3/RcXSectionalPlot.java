...
public class RcXSectionalPlot {
    ...
    private ScatterSpinner spinnerRcH;
    private ScatterSpinner spinnerRcL;
    ...
    public RcXSectionalPlot(Dataset data, String dir){
        ...
        spinnerRcH.setValue(maxCount);

        manualFile.setText(dataset.getFileName());
        ...
        spinnerRcL.setValue(1);
        //spinnerRcH.setValue(maxCount); //last value of original

        spinnerRcL.addChangeListener(new ChangeListener(){
            public void stateChanged(ChangeEvent evt){
                rcSpinnerLChanged();
            }
        });

        spinnerRcH.addChangeListener(new ChangeListener(){
            public void stateChanged(ChangeEvent evt){
                rcSpinnerHChanged();
            }
        });
    ...
    public void rcSpinnerLChanged(){
        ...
        int current = (Integer) spinnerRcL.getValue() - 1;

        if (current < 0){
            spinnerRcL.setValue(1);
            spinnerRcL.setPriorIndex(1);
        } else {
            // add or remove values to guinierDataset
            int direction = (Integer) spinnerRcL.getValue() - spinnerRcL.getPriorIndex();
            int itemCount = 0;
            ...
            if (direction < 0){
                if (direction == -1) {
                    ...
                    int start = spinnerRcL.getPriorIndex() - 2;
                    int stop = ((Integer) spinnerRcL.getValue()).intValue() - 1;
                ...
                if (direction == 1){
                    ...
                    int limit = ((Integer) spinnerRcL.getValue()).intValue() - spinnerRcL.getPriorIndex();
            ...
            maxCount = itemCount;
            spinnerRcL.setPriorIndex((Integer) spinnerRcL.getValue());
            replotRc();
        }
    }
    ...
    public void rcSpinnerHChanged(){
        ...
        int current = (Integer) spinnerRcH.getValue() - 1;
        int totalValues = dataset.getOriginalPositiveOnlyData().getItemCount();
        if (current+1 >= totalValues){
            spinnerRcH.setValue(totalValues);
        }

        // add or remove values to guinierDataset
        int direction = (Integer) spinnerRcH.getValue() - spinnerRcH.getPriorIndex();
        int itemCount;
        ...
        if (direction < 0){
            ...
            if (direction == -1){
                ...
                // 17 - 13
                int stop = ((Integer) spinnerRcH.getValue()).intValue();
                //int size = original.getItemCount();
                int start = spinnerRcH.getPriorIndex() - maxCount;
                XYDataItem tempXY;
                double q2, lnI;
                original.clear();
                for(int i=start; i< stop; i++){
            ...
            if (direction == 1){
                ...
                int start = spinnerRcH.getPriorIndex();
                int stop = current + 1;
                for (int i = start; i<stop; i++){
                    tempXY = dataset.getOriginalPositiveOnlyData().getDataItem(i);
                    q2 = tempXY.getXValue()*tempXY.getXValue();
        ...
        spinnerRcH.setPriorIndex((Integer) spinnerRcH.getValue());
        replotRc();
    }
    ...
    private void createUIComponents() {
        ...
        System.out.println(dataset.getAllData().getMaxX());
        spinnerRcL = new ScatterSpinner(1, dataset.getId());
        spinnerRcH = new ScatterSpinner(dataset.getOriginalPositiveOnlyData().getItemCount(), dataset.getId());
    }
}
